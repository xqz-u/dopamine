...
class SACEncoderNetworkTest(absltest.TestCase):
    ...
    def test_actor_cant_update_conv_weights(self):
        ...
        self.assertNotEqual(jnp.sum(jnp.abs(grad["params"]["Dense_1"]["kernel"])), 0.0)
    ...
    def test_critic_can_update_conv_weights(self):
        ...
        self.assertNotEqual(jnp.sum(jnp.abs(grad["params"]["Dense_0"]["kernel"])), 0.0)

        # It suffices to check the final conv gradients are not 0.
        self.assertNotEqual(jnp.sum(jnp.abs(grad["params"]["Conv_3"]["kernel"])), 0.0)
