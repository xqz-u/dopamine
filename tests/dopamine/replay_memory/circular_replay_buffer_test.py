...
class WrappedReplayBufferTest(tf.test.TestCase):
    ...
    def testConstructorWithZeroUpdateHorizon(self):
        with self.assertRaisesRegexp(ValueError, r"Update horizon must be positive\."):
            circular_replay_buffer.WrappedReplayBuffer(
                observation_shape=OBSERVATION_SHAPE,
                stack_size=STACK_SIZE,
                update_horizon=0,
    ...
    def _verify_sampled_trajectories(self, batch):
        ...
        self.assertAllClose(indices, np.ones(BATCH_SIZE) * BATCH_SIZE, rtol=BATCH_SIZE)
    ...
    def testWrapperLoad(self):
        ...
        self.assertNotEqual(replay.memory._store["observation"], self._test_observation)
        ...
        self.assertAllClose(replay.memory._store["observation"], self._test_observation)
