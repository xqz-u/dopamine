...
class LossesTest(parameterized.TestCase):
    @parameterized.named_parameters(
        ...
        dict(
            testcase_name="MixedArraysDefaultDelta",
            target=onp.ones(5),
            prediction=onp.array([0.0, 1.0, 2.0, 3.0, 4.0]),
            delta=None,
            expected=onp.array([0.5, 0.0, 0.5, 1.5, 2.5]),
        ),
        dict(
            testcase_name="MixedArraysSetDelta",
            target=onp.ones(5),
            prediction=onp.array([0.0, 1.0, 2.0, 3.0, 4.0]),
            delta=2.0,
            expected=onp.array([0.5, 0.0, 0.5, 2.0, 4.0]),
        ),
    )
    def testHuberLoss(
        self,
    ...
    @parameterized.named_parameters(
        dict(
            testcase_name="1DParameters",
            target=2.0,
            prediction=0.0,
            expected=onp.array(4.0),
        ),
        dict(
            testcase_name="ArrayParameters",
            target=onp.ones(5),
            prediction=onp.array([0.0, 1.0, 2.0, 3.0, 4.0]),
