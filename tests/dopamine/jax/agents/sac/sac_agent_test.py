...
def get_mock_batch(
    ...
    mock_observation = np.full(observation_shape + (stack_size,), 1.0, dtype=np.float32)
...
def create_agent(
    action_shape: Union[Tuple[int, ...], int] = 4,
...
def get_agent_params(agent: sac_agent.SACAgent) -> Tuple[flax.core.FrozenDict, ...]:
    return agent.network_params, agent.log_alpha


class SacAgentTest(parameterized.TestCase):
    def assertAgentParametersEqual(
        self, agent1: sac_agent.SACAgent, agent2: sac_agent.SACAgent
    ):
    ...
    def testTupleShapedActionsMatchShapes(
    ...
    def testAgentParametersUpdateWhenTrained(self, action_shape: Tuple[int, ...]):
        ...
        self.assertAgentParametersNotEqual(agent_before_training, agent_after_one_step)
    ...
    def testRestoreAgentFromBundleRestoresParameters(self):
        ...
        agent1._replay.save = mock.create_autospec(agent1._replay.save, spec_set=True)
        agent2._replay.load = mock.create_autospec(agent2._replay.load, spec_set=True)
