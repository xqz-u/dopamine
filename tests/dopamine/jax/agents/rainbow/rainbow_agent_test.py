...
class ProjectDistributionTest(absltest.TestCase):
    ...
    def testExampleFromCodeComments(self):
        ...
        weights = jnp.array(
            [[0.1, 0.6, 0.1, 0.1, 0.1], [0.1, 0.2, 0.5, 0.1, 0.1]], dtype=jnp.float32
        )
    ...
    def testProjectBatchOfDifferentDistributionsWithLargerDelta(self):
        supports = jnp.array([[0, 2, 4, 6, 8], [8, 9, 10, 12, 14]], dtype=jnp.float32)
        weights = jnp.array(
            [[0.1, 0.2, 0.2, 0.2, 0.3], [0.1, 0.2, 0.4, 0.1, 0.2]], dtype=jnp.float32
        )
...
class RainbowAgentTest(absltest.TestCase):
    def setUp(self):
        ...
        self.zero_state = onp.zeros((1,) + self.observation_shape + (self.stack_size,))
    ...
    def testShapesAndValues(self):
        ...
        net_output = agent.network_def.apply(agent.online_params, state, agent._support)
        self.assertEqual(net_output.logits.shape, (self._num_actions, self._num_atoms))
    ...
    def testStepEval(self):
        ...
        onp.array_equal(agent.state, expected_state)
        onp.array_equal(
            agent._last_observation, onp.ones(self.observation_shape) * (num_steps - 1)
        )
    ...
    def testStoreTransitionWithUniformSampling(self):
        agent = rainbow_agent.JaxRainbowAgent(num_actions=4, replay_scheme="uniform")
