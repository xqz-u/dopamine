...
class CheckpointerTest(tf.test.TestCase):
    ...
    def testCheckpointingInitialization(self):
        ...
        with self.assertRaisesRegexp(ValueError, "No path provided to Checkpointer."):
            checkpointer.Checkpointer("")
        # Fails with invalid directory.
        invalid_dir = "/does/not/exist"
        with self.assertRaisesRegexp(
            ValueError, "Unable to create checkpoint path: {}.".format(invalid_dir)
        ):
    ...
    def testLoadLatestCheckpoint(self):
        ...
        self.assertEqual(
            second_iter, checkpointer.get_latest_checkpoint_number(self._test_subdir)
        )
        ...
        for iteration_number in range(total_log_files):
            ...
            for prefix in prefixes:
                checkpoint_file = os.path.join(
                    self._test_subdir, "{}.{}".format(prefix, iteration_number)
                )
    ...
    def testGarbageCollectionWithCheckpointFrequency(self):
        ...
        deleted_log_files = 6
        total_log_files = (
            (checkpointer.CHECKPOINT_DURATION * checkpoint_frequency)
            + deleted_log_files
            + 1
        )
