...
from dopamine.discrete_domains import checkpointer, logger, run_experiment
...
def _create_mock_checkpointer():
    ...
    test_dictionary = {"current_iteration": 1729, "logs": "logs"}
...
class RunExperimentTest(tf.test.TestCase):
    ...
    def testCreateDQNAgent(self, mock_dqn_agent):
        ...
        self.assertEqual(
            ...
            run_experiment.create_agent(
                self.test_session(), environment, agent_name="dqn"
            ),
        )
    ...
    def testCreateRainbowAgent(self, mock_rainbow_agent):
        ...
        self.assertEqual(
            70,
            run_experiment.create_agent(
                self.test_session(), environment, agent_name="rainbow"
            ),
        )
...
class RunnerTest(tf.test.TestCase):
    ...
    def testInitializeCheckpointingWithNoCheckpointFile(self, mock_get_latest):
        ...
        with self.assertRaisesRegexp(tf.errors.PermissionDeniedError, ".*/does.*"):
    ...
    def testInitializeCheckpointingWhenCheckpointUnbundleFails(
        self, mock_logger_constructor, mock_checkpointer_constructor, mock_get_latest
    ):
        ...
        runner = run_experiment.Runner(
            self._test_subdir, lambda x, y, summary_writer: agent, mock.Mock
        )
    ...
    def testInitializeCheckpointingWhenCheckpointUnbundleSucceeds(
        ...
        checkpoint_data = {"current_iteration": current_iteration, "logs": logs_data}
        ...
        runner = run_experiment.Runner(
            self._test_subdir, lambda x, y, summary_writer: mock_agent, mock.Mock
        )
    ...
    def testRunOnePhase(self):
        ...
        for _ in range(calls_to_run_episode):
            expected_statistics.append(
                {"test_episode_lengths": 2, "test_episode_returns": -1}
            )
    ...
    def testLogExperiment(self, mock_logger_constructor):
        ...
        self.assertEqual(
            (num_iterations / log_every_n), experiment_logger._calls_to_log
        )

    @mock.patch.object(checkpointer, "Checkpointer")
    @mock.patch.object(logger, "Logger")
    def testCheckpointExperiment(
    ...
    def testRunExperimentWithInconsistentRange(
        ...
        mock_checkpointer_constructor.return_value = experiment_checkpointer
        runner = run_experiment.Runner(
            self._test_subdir, self._create_agent_fn, mock.Mock, num_iterations=0
        )
    ...
    def testRunExperiment(
        ...
        runner.run_experiment()
        self.assertEqual(
            num_iterations, experiment_checkpointer.save_checkpoint.call_count
        )
        self.assertEqual(num_iterations, experiment_logger._calls_to_set)
