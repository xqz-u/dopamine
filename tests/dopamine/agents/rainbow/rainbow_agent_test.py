...
class ProjectDistributionTest(tf.test.TestCase):
    ...
    def testInconsistentSupportsAndWeightsWithPlaceholders(self):
        ...
        with self.test_session() as sess:
            ...
            with self.assertRaisesRegexp(
                tf.errors.InvalidArgumentError, "assertion failed"
            ):
                sess.run(
                    projection,
                    feed_dict={
    ...
    def testInconsistentSupportsAndTargetSupportParameters(self):
        supports = tf.constant([[0, 2, 4, 6, 8], [3, 4, 5, 6, 7]], dtype=tf.float32)
        weights = tf.constant(
            [[0.1, 0.2, 0.3, 0.2, 0.2], [0.1, 0.2, 0.3, 0.2, 0.2]], dtype=tf.float32
        )
    ...
    def testExampleFromCodeComments(self):
        ...
        weights = tf.constant(
            [[0.1, 0.6, 0.1, 0.1, 0.1], [0.1, 0.2, 0.5, 0.1, 0.1]], dtype=tf.float32
        )
        target_support = tf.constant([4, 5, 6, 7, 8], dtype=tf.float32)
        projection = rainbow_agent.project_distribution(
            supports, weights, target_support
        )
        expected_projections = [[0.8, 0.0, 0.1, 0.0, 0.1], [0.8, 0.1, 0.1, 0.0, 0.0]]
        with self.test_session() as sess:
            tf.compat.v1.global_variables_initializer().run()
            projection_ = sess.run(projection)
    ...
    def testProjectBatchOfDifferentDistributionsWithLargerDelta(self):
        supports = tf.constant([[0, 2, 4, 6, 8], [8, 9, 10, 12, 14]], dtype=tf.float32)
        weights = tf.constant(
            [[0.1, 0.2, 0.2, 0.2, 0.3], [0.1, 0.2, 0.4, 0.1, 0.2]], dtype=tf.float32
        )
        target_support = tf.constant([0, 4, 8, 12, 16], dtype=tf.float32)
        projection = rainbow_agent.project_distribution(
            supports, weights, target_support
        )
        expected_projections = [[0.2, 0.4, 0.4, 0.0, 0.0], [0.0, 0.0, 0.45, 0.45, 0.1]]
        with self.test_session() as sess:
            tf.compat.v1.global_variables_initializer().run()
            projection_ = sess.run(projection)
            self.assertAllClose(expected_projections, projection_)
...
class RainbowAgentTest(tf.test.TestCase):
    def setUp(self):
        ...
        self.zero_state = np.zeros((1,) + self.observation_shape + (self.stack_size,))
    ...
    def testShapesAndValues(self):
        with tf.compat.v1.Session() as sess:
            ...
            self.assertEqual(self.evaluate(tf.reduce_min(agent._support)), -self._vmax)
            self.assertEqual(self.evaluate(tf.reduce_max(agent._support)), self._vmax)
            self.assertEqual(
                agent._net_outputs.logits.shape, (1, self._num_actions, self._num_atoms)
            )
            self.assertEqual(
                agent._net_outputs.probabilities.shape, agent._net_outputs.logits.shape
            )
            self.assertEqual(
                agent._replay_net_outputs.logits.shape[1], self._num_actions
            )
            self.assertEqual(agent._replay_net_outputs.logits.shape[2], self._num_atoms)
            self.assertEqual(
                agent._replay_next_target_net_outputs.logits.shape[1], self._num_actions
            )
            self.assertEqual(
                agent._replay_next_target_net_outputs.logits.shape[2], self._num_atoms
            )
            self.assertEqual(agent._net_outputs.q_values.shape, (1, self._num_actions))
    ...
    def testStepTrain(self):
        ...
        with tf.compat.v1.Session() as sess:
            ...
            self.assertAllEqual(agent.state, expected_state)
            self.assertAllEqual(
                agent._last_observation, np.full(self.observation_shape, num_steps - 1)
            )
            self.assertAllEqual(agent._observation, observation[:, :, 0])
    ...
    def testStoreTransitionWithUniformSampling(self):
        with tf.compat.v1.Session() as sess:
            ...
            agent._store_transition(dummy_frame, 0, 0, False, 10.0)
            agent._store_transition(dummy_frame, 0, 0, False)
