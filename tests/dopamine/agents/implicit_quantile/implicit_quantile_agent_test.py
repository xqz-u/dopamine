...
class ImplicitQuantileAgentTest(tf.test.TestCase):
    def setUp(self):
        ...
        self.ones_state = np.ones((1,) + self.observation_shape + (self.stack_size,))
    ...
    def testShapes(self):
        with self.test_session(use_gpu=False) as sess:
            ...
            self.assertEqual(
                agent._net_outputs.quantile_values.shape[0], agent.num_quantile_samples
            )
            self.assertEqual(
    ...
    def test_q_value_computation(self):
        with self.test_session(use_gpu=False) as sess:
            ...
            q_values, q_argmax = sess.run([agent._q_values, agent._q_argmax], feed_dict)
