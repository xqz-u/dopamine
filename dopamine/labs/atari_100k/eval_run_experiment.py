...
class MaxEpisodeEvalRunner(run_experiment.Runner):
    ...
    def __init__(self, base_dir, create_agent_fn, num_eval_episodes=100, max_noops=30):
    ...
    def _run_no_ops(self):
        ...
        num_noops = jax.random.randint(rng, shape=(), minval=0, maxval=self._max_noops)
        ...
        if self._environment.environment.ale.game_over():
            ...
            observation = (
                self._environment._pool_and_resize()
            )  # pylint: disable=protected-access
        return observation
