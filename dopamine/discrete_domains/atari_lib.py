...
class NatureDQNNetwork(tf.keras.Model):
    ...
    def __init__(self, num_actions, name=None):
        ...
        self.conv1 = tf.keras.layers.Conv2D(
            32, [8, 8], strides=4, padding="same", activation=activation_fn, name="Conv"
        )
        self.conv2 = tf.keras.layers.Conv2D(
            64, [4, 4], strides=2, padding="same", activation=activation_fn, name="Conv"
        )
        self.conv3 = tf.keras.layers.Conv2D(
            64, [3, 3], strides=1, padding="same", activation=activation_fn, name="Conv"
        )
