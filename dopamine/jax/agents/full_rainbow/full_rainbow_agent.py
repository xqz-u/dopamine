...
def select_action(
    ...
    epsilon = jnp.where(
        ...
        epsilon_eval,
        epsilon_fn(
            epsilon_decay_period, training_steps, min_replay_history, epsilon_train
        ),
    )
...
class JaxFullRainbowAgent(dqn_agent.JaxDQNAgent):
    ...
    def __init__(
        ...
        logging.info("\t num_updates_per_train_step: %d", num_updates_per_train_step)
    ...
    def _training_step_update(self):
        ...
        (self.optimizer_state, self.online_params, loss, mean_loss, self._rng) = train(
            self.network_def,
            self.online_params,
    ...
    def _store_transition(
        ...
        *args,
        ...
        is_prioritized = isinstance(
            self._replay, prioritized_replay_buffer.OutOfGraphPrioritizedReplayBuffer
        )
