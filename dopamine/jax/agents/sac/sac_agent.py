...
from dopamine.jax import continuous_networks, losses
...
def train(
    ...
    def loss_fn(
        ...
        target_q_value = jnp.squeeze(jnp.minimum(target_q_value_1, target_q_value_2))
        ...
        no_grad_q_value = jnp.squeeze(jnp.minimum(q_value_no_grad_1, q_value_no_grad_2))
    ...
    (_, aux_vars), gradients = grad_fn(
        network_params, log_alpha, states, actions, rewards, next_states, terminals, rng
    )
...
class SACAgent(dqn_agent.JaxDQNAgent):
    ...
    def __init__(
        ...
        logging.info(
            "Creating %s agent with the following parameters:", self.__class__.__name__
        )
    ...
    def _maybe_sync_weights(self):
        ...
        self.target_params = jax.tree_multimap(
