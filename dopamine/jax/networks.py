...
gin.constant(
    "jax_networks.CARTPOLE_MIN_VALS", (-2.4, -5.0, -onp.pi / 12.0, -onp.pi * 2.0)
)
gin.constant("jax_networks.CARTPOLE_MAX_VALS", (2.4, 5.0, onp.pi / 12.0, onp.pi * 2.0))
gin.constant("jax_networks.ACROBOT_OBSERVATION_DTYPE", jnp.float64)
gin.constant("jax_networks.ACROBOT_MIN_VALS", (-1.0, -1.0, -1.0, -1.0, -5.0, -5.0))
...
class ClassicControlDQNNetwork(nn.Module):
    ...
    def setup(self):
        ...
        self.final_layer = nn.Dense(features=self.num_actions, kernel_init=initializer)
...
class ImplicitQuantileNetwork(nn.Module):
    ...
    def __call__(self, x, num_quantiles, rng):
        ...
        quantile_net = nn.Dense(features=state_vector_length, kernel_init=initializer)(
            quantile_net
        )
        ...
        x = nn.Dense(features=512, kernel_init=initializer)(x)
        x = nn.relu(x)
        quantile_values = nn.Dense(features=self.num_actions, kernel_init=initializer)(
            x
        )
