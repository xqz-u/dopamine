...
from dopamine.jax.agents.dqn import dqn_agent as dqn
...
action = agent_dqn.begin_episode(env.reset())
...
for _ in range(steps):
    ...
    if done:
        ...
        print("------------------------------------------------")
        agent_dqn.end_episode(reward, done)
        agent.end_episode(reward, done)
        action = agent.begin_episode(env.reset())
        partial_steps, total_reward = 1, 0
    else:
        action = agent.step(reward, observation)
        action = agent_dqn.step(reward, observation)
        partial_steps += 1
